...
export default async function handler(req, res) {
  ...
  for (const row of dataPag) {
    const data = new Date(row[idxDataPrevista])
    ...
    if (!isNaN(data)) {
      ...
      if (status === "recebido" && rowAno === +ano) totalRecebidoAno += valor
      if (status !== "recebido" && rowAno === +ano) totalReceberAno += valor
      if (status !== "recebido" && rowAno === +ano && rowMes === +mes) totalReceberMes += valor
    }
  }
