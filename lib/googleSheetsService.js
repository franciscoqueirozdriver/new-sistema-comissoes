...
import { HOLERITE_COLUMNS, HOLERITES_SHEET_NAME } from "@/lib/constants/holerites";
...
/**
 * Escreve uma linha na aba de Holerites usando a ordem oficial das 15 colunas.
 * Nenhuma conversão numérica é realizada.
 * @param {Record<string, string>} data - Objeto com as 15 chaves do holerite.
 */
export async function upsertHoleriteRow(data) {
  const row = HOLERITE_COLUMNS.map((k) => data[k] ?? "");
  await appendRows(HOLERITES_SHEET_NAME, [row]);
}
