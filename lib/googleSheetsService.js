...
export async function getSheetData(sheetName) {
  ...
  // Padroniza o cabeçalho para minúsculas e sem espaços para facilitar buscas futuras
  const header = allRows[0].map(h => String(h || "").toLowerCase().trim());
...
export async function appendRows(sheetName, dataRows) {
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${sheetName}!A1`,
    valueInputOption: "USER_ENTERED", // Permite que o Sheets interprete os dados (ex: formatos de data)
    insertDataOption: "INSERT_ROWS", // Insere as linhas em vez de sobrescrever
    requestBody: {
      values: dataRows,
    },
  });
...
export async function updateSheet(sheetName, data) {
    ...
    if (data && data.length > 0) {
        await sheets.spreadsheets.values.update({
            spreadsheetId: SPREADSHEET_ID,
            range: `${sheetName}!A1`, // Começa a escrever na célula A1
